...
public class VerticalFlowLayout implements LayoutManager, java.io.Serializable {
	...
	private static final long serialVersionUID = 726819700058814505L;
	int _halign;
	int _valign;
	int _hgap;
	int _vgap;
	 ...
	 *            Description of Parameter
	 */
	public VerticalFlowLayout(final int halign, final int valign) {
	 ...
	 *            Description of Parameter
	 */
	public VerticalFlowLayout(final int halign, final int valign, final int hgap, final int vgap) {
		_hgap = hgap;
		_vgap = vgap;
	 ...
	 *            The new Alignment value
	 */
	public void setAlignment(final int halign, final int valign) {
		_halign = halign;
		_valign = valign;
	}
	...
	public Dimension preferredLayoutSize(final Container target) {
		synchronized (target.getTreeLock()) {
			...
			for (int ii = 0; ii < nmembers; ii++) {
				final Component m = target.getComponent(ii);
				if (m.isVisible()) {
	...
	public void layoutContainer(final Container target) {
		synchronized (target.getTreeLock()) {
			final Insets insets = target.getInsets();
			final int maxheight = target.getHeight() - (insets.top + insets.bottom + _vgap * 2);
			final int nmembers = target.getComponentCount();
			int y = 0;

			final Dimension preferredSize = preferredLayoutSize(target);
